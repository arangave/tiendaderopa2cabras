// /app/components/InicioCliente.tsx
"use client";
...
interface InicioClienteProps {
  ...
  destacados: Product[];
  rebelTshirts: Product[];
  rebelSweaters: Product[];
  naughtyTshirts: Product[];
  naughtySweaters: Product[];
}
...
export default function InicioCliente({
  ...
  // Construir arrays de productos a partir de las props
  ...
  // Combinar prop unsubscribed y query string para mostrar mensaje
  useEffect(() => {
    if (unsubscribed) {
      setUnsubMsg("Te has dado de baja correctamente de la newsletter.");
      return;
    }
    const param = searchParams.get("unsubscribed");
    if (param === "1") {
      setUnsubMsg("Te has dado de baja correctamente de la newsletter.");
    } else if (param === "0") {
  ...
  // Quitar mensaje tras 5 segundos
  useEffect(() => {
    if (unsubMsg) {
  ...
  // Detectar ancho de ventana para itemsPerPage
  useEffect(() => {
  ...
  // Leer tema del localStorage y aplicarlo
  useEffect(() => {
  ...
  // Efecto para tachar texto con clase “norma-hover”
  useEffect(() => {
      ...
      <main>
          ...
          >
            <li>
              <a
                ...
                className="font-semibold hover:text-[#67b2c1]"
              >
                Inicio
              </a>
            </li>
        ...
        <section className="hero pt-4 sm:pt-17 pb-4 sm:pb-8">
          ...
          <div className="hero-content px-2 sm:px-8 max-w-7xl mx-auto mt-[-50px] sm:mt-0">
            <div className="hero-text text-center sm:text-left">
              <h2 className="text-xs sm:text-sm font-bold mb-1 sm:mb-3">
                Descubre el estilo de 2CabrasConTraje
              </h2>
              <p className="text-[8px] sm:text-[10px] mb-1 sm:mb-2">
          ...
          <div className="relative max-w-7xl mx-auto px-4">
            ...
            {sweaters.length > itemsPerPage && (
              <button
                ...
                disabled={sweaters.length <= itemsPerPage}
              >
                ›
              </button>
            )}
          ...
          <div className="relative max-w-7xl mx-auto px-4">
            ...
            {tshirts.length > itemsPerPage && (
              <button
                ...
                className="absolute right-0 top-1/2 -translate-y-1/2 bg-black text-white p-2 rounded-full z-10 hover:bg-gray-700"
                disabled={tshirts.length <= itemsPerPage}
              >
                ›
              </button>
            )}
        ...
        <section className="bg-white py-12 px-4 text-center">
          <h2 className="text-3xl font-bold mb-4 text-black">
            Únete al mundo de{" "}
            <span className="bg-gradient-to-r from-[#67b2c1] via-[#ff8eaa] to-[#f6bd6b] bg-clip-text text-transparent">
              2CabrasConTraje
            </span>
          </h2>
          <p className="text-gray-700 max-w-xl mx-auto mb-6">
            Recibe ideas, filosofía de marca y alguna que otra forma de
            <br /> &quot;poner los cuernos&quot;... al sistema. ✨
          </p>
